...
class TimestampFormatGet extends Action {
    ...
    public boolean perform(final Context context) {
        ...
        Main.messageManager.send(context.sender, "Timestamp format for " + targetName + ": " + TimestampFormatGet.message(recipient), MessageLevel.STATUS, false);
        return true;
    }

    static String message(final Recipient recipient) {
        return recipient.getFormat().replaceAll("&", "&&") + " (%1$s=msg*, %2$s=time)";
    }
